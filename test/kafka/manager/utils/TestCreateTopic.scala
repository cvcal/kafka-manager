...
class TestCreateTopic extends CuratorAwareTest {
  ...
  test("create topic") {
    withCurator { curator =>
      ...
      val td = TopicIdentity.from(3,TopicDescription("mytopic",(stat.getVersion(),json),None,Map.empty,Option((-1,configJson)),false),None,defaultClusterConfig)
  ...
  test("alter topic - add partitions") {
    withCurator { curator =>
      ...
      //check partitions were added and config updated
      {
        val json: String = curator.getData.forPath(ZkUtils.getTopicPath("mytopic"))
        val configJson: String = curator.getData.forPath(ZkUtils.getTopicConfigPath("mytopic"))
        val td = TopicIdentity.from(3, TopicDescription("mytopic", (-1,json), None, Map.empty, Option((-1,configJson)), false),None, defaultClusterConfig)
  ...
  test("alter topic - update config") {
    withCurator { curator =>
      ...
      val td = TopicIdentity.from(3,TopicDescription("mytopic",(stat.getVersion,json),None,Map.empty,Option((configReadVersion,configJson)),false),None, defaultClusterConfig)
      val properties = new Properties()
      ...
      //check config
      {
        ...
        val configStat = new Stat
        val configJson : String = curator.getData.storingStatIn(configStat).forPath(ZkUtils.getTopicConfigPath("mytopic"))
        val td = TopicIdentity.from(3, TopicDescription("mytopic", (-1,json), None, Map.empty, Option((configStat.getVersion,configJson)), false),None, defaultClusterConfig)
